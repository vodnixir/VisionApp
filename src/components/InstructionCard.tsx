...
export function InstructionCard({ title, subtitle, rules, startLabel, onStart, onBack }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="flex max-h-[92vh] w-full max-w-md flex-col overflow-y-auto rounded-3xl border border-edge bg-page p-6 shadow-2xl landscape:max-h-[94vh] landscape:p-4">
        <header className="mb-4 text-center landscape:mb-2">
        ...
        <ul className="mb-6 flex flex-col gap-3 landscape:mb-3 landscape:gap-2">
          {rules.map((r, i) => (
            <li
              key={i}
              className="flex items-center gap-3 rounded-2xl border border-edge bg-card px-4 py-3 landscape:py-2"
            >
